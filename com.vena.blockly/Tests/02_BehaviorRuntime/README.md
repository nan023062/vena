...
- `TimelineSignalDemo` — `LogicClip`（onBegin / onFrame / onEnd 各驱动 `LogSignalSource`）+ 指定帧 `Signal`，默认 `clipDuration = 0.5s @ 30fps`，预期 `clip begin → clip frame ×15 → signal at frame 5 → clip end`，16 tick 完成
