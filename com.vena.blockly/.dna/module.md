...
### 7. 作用域链作为双图共享基础 (Phase 1: 抽出为独立子模块、对外签名零修改)
...
调试 / 断点 / 单步 / 值监视通道是 Phase 2 的独立接口，**不复用 `IBlocklyLogger`**。`IBlocklyLogger` 仅用于单向日志记录，不要把暂停/恢复回调塞进 logger。

## Phase 1 Ratchet

第一阶段锁定的工程范围（完整契约见 `contract.md`）。**做**三件事：(1) 把 ScopeChain 从 `Blockly` 中抽出为独立子模块——业务方调用代码零修改；(2) `IProcedureImpl` / `IFunctionImpl` 补齐至 N=5 参（基类包装同步补齐）；(3) `IBehaviorNode.Tick` 返回值改双态枚举 `BehaviorResult { Running / Done }`，已知行为节点 Tick 实现全部迁移。双态对齐 imperative 流图范式（非 BT）：分支决策由 LogicGraph 表达式完成，节点返回值只表「完成 / 未完成」。**不做**：命名空间改名、物理目录搬迁、Wait/Delay 节点、协程/Task/异步、反射注册表、注解扫描、codegen——这些保持现状或留待后续阶段。
