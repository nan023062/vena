...
11. **Blockly 三层架构（按工作方式切分）**：
    - **Layer 1 — 人手写维护**：包心运行时（`IBlocklySource` / `Expression` / `Block` / 求值器 / IR Loader）+ 节点 Impl（`[BlocklyCodeGenMethod]` 标注的核心算法）。维护方式 = 人写、AI 辅助、人 review。
    - **Layer 2 — 工具自动化**：① **AutoGen Codegen**（`[BlocklySource]` 注解扫描 → `Source` / `Source.Node` / `Provider` 三件套 `.g.cs`）；工具本身 Editor-only（`UgcCodeWriter` / `UgcAnnotationScanner` / `UgcCodegenMenu`），产物进 runtime asmdef、被 `BehaviorGraph` / `LogicGraph` 实例化执行。② **IR Compiler**（图配置 ↔ source 树双向转换）；编辑期 = `JsonGraphSerializer`（IR → JSON），runtime = `GraphLoader`（JSON → source 树，**必须能在 runtime UGC 玩家场景跑**）。维护方式 = ① 人 0 维护（AutoGen 产物挂 `<auto-generated>` 头）、② 工程内基线。
...
15. **Phase 3 AOT 暂搁置**：原 Phase 3「IR → C# 编译期固化」**放弃为产品路径**。
...
- **Phase 2** = codegen（扫 `[BlocklyCodeGenMethod]` / `[BlocklyCodeGenMember]` / `[BlocklySource]` 注解导出 Impl 扩展代码）+ 编辑器配置工具
