...
11. **Blockly 三层架构（按工作方式切分）**：
    - **Layer 1 — 人手写维护**：包心运行时（`IBlocklySource` / `Expression` / `Block` / 求值器 / IR Loader）+ 节点 Impl（`[UgcMethod]` 标注的核心算法）。维护方式 = 人写、AI 辅助、人 review。
    - **Layer 2 — 工具自动化**：① **AutoGen Codegen**（`[UgcSource]` 注解扫描 → `Source` / `Source.Node` / `Provider` 三件套 `.g.cs`）；工具本身 Editor-only（`UgcCodeWriter` / `UgcAnnotationScanner` / `UgcCodegenMenu`），产物进 runtime asmdef、被 `BehaviorGraph` / `LogicGraph` 实例化执行。② **IR Compiler**（图配置 ↔ source 树双向转换）；编辑期 = `JsonGraphSerializer`（IR → JSON），runtime = `GraphLoader`（JSON → source 树，**必须能在 runtime UGC 玩家场景跑**）。维护方式 = ① 人 0 维护（AutoGen 产物挂 `<auto-generated>` 头）、② 工程内基线。
