...
namespace Vena.Blockly
{
    ...
    /// <summary>
    /// 调试通道 v0（Editor/UI 合约 §4 锁定语义；物理落 Runtime 以保 Editor → Runtime 单向依赖）。
    ///
    /// 范围（v0）：仅三事件、单线程、同步回调。
    /// 不做：时间轴 / 历史回放 / 远程调试。
    ///
    ...
    /// 两者不复用、不继承、不合并。
    ///
    /// **不进父合约 §6 聚合门面**：IBlocklyHost 不暴露此接口；调试通道由 BlocklyEditorWindow
    /// 在编辑器期通过 host 之外的注入路径挂入 Runtime 节点执行链（PR-9 v0 = 静态注入点）。
    /// </summary>
