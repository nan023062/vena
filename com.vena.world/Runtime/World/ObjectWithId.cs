...
namespace Vena
{
    ...
    public abstract class ObjectWithId
    {
        ...
        private static List<FieldInfo> GetAllInstanceFields(Type type)
        {
            ...
            while (type != null && type != typeof(ObjectWithId))
            {
                ...
                foreach (var fieldInfo in infos)
                {
                    ...
                    if (!HasUnitySerializeField(fieldInfo))
                    {
        ...
        private static bool HasUnitySerializeField(FieldInfo fieldInfo)
        {
            foreach (var attributeData in fieldInfo.CustomAttributes)
            {
                if (attributeData.AttributeType.FullName == "UnityEngine.SerializeField")
                {
                    return true;
                }
            }
